...
make_enum_entry <- function(df) {
  df_str <- df %>%
    # mutate(str=glue("\\item \\textit{{{clean_title(Title)}}}, {Year}. \\href{{{Video1}}}{{\\url{{{clean_video(Video1)}}}}}"))
    mutate(str=str_glue("\\item \\textbf{{{clean_title(Title)}}} ({duration}), {month(date)}/{year(date)}. ",
                    "\\href{{{Video1}}}{{\\url{{{clean_video(Video1)}}}}}"))
  df_coll <- df_str$str %>% str_c(collapse="\n")
  str_glue("\\begin{{enumerate}}[resume]\n{df_coll}\n\\end{{enumerate}}\n")
}

make_center_video <- function(fname) {
  str_glue("\n\n\\begin{{center}}",
  ...
  df_str <- df %>%
    ...
    mutate(
      ...
      str=str_glue("\\item \\textbf{{{clean_title(Title)}}} ",
                   "({duration}), {month(date)}/{year(date)}. ",
                   "\\href{{{Video1}}}{{\\url{{{clean_video(Video1)}}}}}",
                   "{make_center_video(video_fname)}\n",
...
make_subsec_entry_long <- function(df) {
  df_str <- df %>%
    ...
    mutate(str=str_glue("\n\\subsection{{{clean_title(Title)}}}\n",
                    "Duration: {duration}. Published: {month(date)}/{year(date)}. N: {N}. ",
                    "\\href{{{Video1}}}{{\\url{{{clean_video(Video1)}}}}}\n\n",
                    "{clean_title(descr)}\n"))
  df_coll <- df_str$str %>% str_c(collapse="\n")
  df_coll
